...
export default function DashboardNav({ role, email }: DashboardNavProps) {
  ...
  const handleLogout = async () => {
    ...
    try {
      ...
      // Supabase not configured — fine in demo mode
    }
    router.push('/login');
    router.refresh();
  };
  ...
  return (
    <nav className="flex items-center justify-between px-6 py-4 border-b border-white/10 glass-panel sticky top-0 z-50">
      <div className="flex items-center gap-3">
        ...
        <span className={`px-2.5 py-0.5 text-xs font-semibold rounded-full capitalize border ${roleColors[role] ?? 'text-slate-400 bg-slate-800 border-slate-600'}`}>
          {role}
        </span>
      </div>
