...
import { classifyRootCause } from '@/lib/rootCauseEngine';
import {
...
export default async function StudentProfilePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const riskAssessment = calculateRiskScore({
    attendance: { currentRate: attendNow ?? undefined, previousRate: attendPrev ?? undefined },
    academics:  { currentScore: gpaNow ?? undefined,  previousScore: gpaPrev ?? undefined },
    engagement: {
  ...
  const engineRootCause = classifyRootCause({
    attendance: attendNow ?? undefined,
    attendancePrev: attendPrev ?? undefined,
    academicScore: gpaNow ?? undefined,
    academicScorePrev: gpaPrev ?? undefined,
    lmsLogins: lmsNow ?? undefined,
    lmsLoginsPrev: lmsPrev ?? undefined,
    assignmentsSubmitted: assNow ?? undefined,
    assignmentsTotal: assTotal,
    attendanceStable: attendNow !== null && attendPrev !== null ? Math.abs(attendNow - attendPrev) <= 3 : undefined,
    academicStable: gpaNow !== null && gpaPrev !== null ? Math.abs(gpaNow - gpaPrev) <= 4 : undefined,
  });

  // DB overrides label + confidence if available
  const rc = rcRows?.[0] ?? null;
  const rcFactors: string[] = (rc?.factors?.length ? (rc.factors as string[]) : null) ?? engineRootCause.contributing;
  const rootCause = {
    displayLabel:     rc ? rc.primary_cause : engineRootCause.displayLabel,
    confidence:       rc ? rc.confidence    : engineRootCause.confidence,
    factors:          rcFactors,
    altCauses:        engineRootCause.altCauses,
    explanation:      engineRootCause.explanation,
    interventions:    engineRootCause.recommendedInterventions,
    dataCompleteness: engineRootCause.dataCompleteness,
  };
    ...
    <div className="min-h-screen">
      <div className="border-b border-white/8 bg-slate-950/60 backdrop-blur sticky top-0 z-40">
        <div className="max-w-5xl mx-auto px-4 md:px-8 py-3 flex items-center gap-3">
          <Link href="/dashboard/faculty" className="flex items-center gap-1.5 text-xs font-medium text-slate-400 hover:text-white transition-colors">
      ...
      <div className="max-w-5xl mx-auto px-4 md:px-8 py-8 space-y-10">
        ...
        <Section title="Current Indicators" icon={<Activity className="w-4 h-4" />}>
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            <MetricCard label="Attendance" value={attendNow} unit="%" prevValue={attendPrev} prevLabel="Prev 4 weeks" icon={<Calendar className="w-4 h-4" />} alert={attendAlert} />
            <MetricCard label="Avg Score" value={gpaNow} unit="%" prevValue={gpaPrev} prevLabel="Prev period" icon={<BookOpen className="w-4 h-4" />} alert={gpaAlert} />
            <MetricCard label="LMS Logins / week" value={lmsNow} prevValue={lmsPrev} prevLabel="Prev period" icon={<BarChart2 className="w-4 h-4" />} alert={lmsAlert} />
            <MetricCard label={`Assignments (of ${assTotal})`} value={assNow} prevValue={assPrev} prevLabel="Prev period" icon={<CheckCircle2 className="w-4 h-4" />} alert={assAlert} />
          </div>
        </Section>
          ...
          <div className="glass-panel rounded-2xl p-6 space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              <div className="flex flex-col gap-3">
                ...
                <div className="text-5xl font-extrabold text-white tabular-nums">{pct(scoreBarWidth)}</div>
              ...
              <div className="flex flex-col gap-3">
                ...
                <div className={`inline-flex px-5 py-3 rounded-xl text-2xl font-extrabold border ${TIER_BADGE[risk.tier]}`}>{risk.tier}</div>
            ...
            {risk.contributingFactors && (
              <div className="pt-4 border-t border-white/5">
                <p className="text-xs font-semibold uppercase tracking-wide text-slate-500 mb-3">Multi-Factor Engine Component Breakdown</p>
                  ...
                  {[
                    ...
                    <div key={factor.name} className="bg-slate-900/60 rounded-xl p-3 border border-white/5">
                      ...
                      <div className="w-full h-1.5 rounded-full bg-white/5 overflow-hidden">
                        <div
                          className={`h-full rounded-full ${factor.val > 0.65 ? 'bg-red-500' : factor.val > 0.40 ? 'bg-amber-500' : 'bg-emerald-500'}`}
        ...
        <Section title="Root Cause Classification" icon={<Brain className="w-4 h-4" />}>
          <div className="space-y-4">
            <div className="glass-panel rounded-2xl p-6 border-l-4 border-indigo-500">
              <div className="flex items-start justify-between gap-4 flex-wrap">
                ...
                <div className="text-right shrink-0">
                  <p className="text-xs text-slate-500 mb-1">Data completeness</p>
                  <div className="flex items-center gap-2 justify-end">
                    <div className="w-20 h-1.5 rounded-full bg-white/5 overflow-hidden">
                      <div className="h-full rounded-full bg-indigo-500/60" style={{ width: `${rootCause.dataCompleteness}%` }} />
                    </div>
                    <span className="text-xs font-mono text-slate-400">{rootCause.dataCompleteness}%</span>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="glass-panel rounded-2xl p-6">
              <p className="text-xs font-semibold uppercase tracking-wide text-slate-500 mb-4">Evidence signals</p>
              <ul className="space-y-2.5">
                {rootCause.factors.map((f: string, i: number) => (
            ...
            {rootCause.altCauses.length > 0 && (
              <div className="glass-panel rounded-2xl p-6">
                <p className="text-xs font-semibold uppercase tracking-wide text-slate-500 mb-4">Alternative causes considered &amp; rejected</p>
                <div className="space-y-3">
                  {rootCause.altCauses.map((ac: { cause: string; confidence: number; reason: string }, i: number) => (
                    <div key={i} className="flex items-start justify-between gap-4">
                      ...
                      <span className="text-xs font-bold text-slate-500 bg-slate-800 px-2 py-0.5 rounded-full whitespace-nowrap">{ac.confidence}%</span>
                    </div>
                  ))}
                </div>
              </div>
        ...
        <Section title="Recommended Interventions" icon={<Lightbulb className="w-4 h-4" />}>
          <div className="glass-panel rounded-2xl p-6 space-y-5">
            <div className="flex flex-wrap gap-2">
              {rootCause.interventions.map((iv: string, i: number) => (
                <span key={i} className="px-3 py-1.5 bg-indigo-500/10 border border-indigo-500/20 text-indigo-300 text-sm font-medium rounded-xl">
                  {iv}
                </span>
              ))}
            </div>
            <div className="flex items-center justify-between flex-wrap gap-4 pt-2 border-t border-white/5">
              <div className="space-y-0.5">
                <p className="text-xs text-slate-500">Engine recommendation confidence</p>
                <div className="flex items-center gap-2">
                  <div className="w-32 h-1.5 rounded-full bg-white/5 overflow-hidden">
                    <div className="h-full rounded-full bg-indigo-500" style={{ width: `${rootCause.confidence}%` }} />
                  </div>
                  <span className="text-xs font-medium text-indigo-400">{rootCause.confidence}%</span>
                </div>
              </div>
              <AssignInterventionModal
                studentId={Number(id)}
                recommendation={rootCause.interventions[0] ?? 'Support Intervention'}
              />
            </div>
          </div>
        </Section>
